...
'''
#embedded encoding drivers
#one-hot encoding tyre compounds

#images to 224, 224 (using opencv)


#cnn to process image data
#lstm to predict trends


   
#20min b4 race --> minute data
...
for location in past_races['Location']:
        ...
        #print(race.session_start_time) time in time deltas
        ...
        Laps_WC['Slick'] = Laps_WC['Compound'].map(hardness_map)
        Laps_WC['Slick'] = Laps_WC['Slick'].fillna(0)
        
        Laps_WC['Wet'] = Laps_WC['Compound'].map(wet_map)
        Laps_WC['Wet'] = Laps_WC['Wet'].fillna(0)

        Laps_WC['Inter'] = Laps_WC['Compound'].map(inter_map)
        Laps_WC['Inter'] = Laps_WC['Inter'].fillna(0)
        
        Laps_WC ['LapTime_sec'] = Laps_WC ['LapTime'].dt.total_seconds()
        print (Laps_WC[['Slick', 'Wet', 'Inter']])
        #deal with NaNs
        Laps_WC['SpeedFL'].ffill(inplace=True)
        Laps_WC['SpeedFL'].bfill(inplace=True)# type: ignore # Critical step to match original index
        Laps_WC['SpeedST'].ffill(inplace=True)
        Laps_WC['SpeedST'].bfill(inplace=True) # type: ignore
        Laps_WC['LapTime_sec'].ffill(inplace=True)
        Laps_WC['LapTime_sec'].bfill(inplace=True) # type: ignore

        Laps_WC['TotalTime'] = Laps_WC.groupby('Driver')['LapTime_sec'].cumsum() #cumulative sum of laptimes
          # Drops rows where these cols are NaN
        ...
        Laps_WC['Sector3Time'] = Laps_WC.groupby('Driver')['Sector3Time'].ffill()
        ...
        Laps_WC = Laps_WC.drop(['Stint', 'DriverNumber', 'IsPersonalBest', 'LapStartDate', 
                                ...
                                'SpeedI1', 'SpeedI2', 'FreshTyre', 'Compound'], axis=1)
...
train.to_csv('All_Laps_Train', index=False)
...
print(unique_values)
val.to_csv('All_Laps_val', index=False)
test = All_Laps[All_Laps['Name'].isin(test_races)]
test.to_csv('All_Laps_test', index=False)
...
'''
        ...
        '''
# Check if NaN TyreLife rows coincide with PitInTime/PitOutTime
